...
/*
double add(double a, double b, double temp, double machin)
{
    printf("\ta = %f\n", a);
    printf("\tb = %f\n", b);
    printf("\ttemp = %f\n", temp);
    printf("\tmachin = %f\n", machin);
    double res = temp + a + b + machin;
    printf("\n\tres = %f\n\n", res);
    printf("\ta = %f\n", a);
    printf("\tb = %f\n", b);
    printf("\ttemp = %f\n", temp);
...
int main() 
{
    ...
    printf("res before = %.2f\n", res);

    #pragma omp parallel for reduction(+:res) num_threads(4)
    for(int i = 0; i < truc; ++i){
        printf("\tbefore i = %d, res = %f, Thread rank: %d\n", i, res, omp_get_thread_num());
        res += i;
        printf("\tafter i = %d, res = %f, Thread rank: %d\n", i, res, omp_get_thread_num());
    }
...
int main()
{
    double values[8] __attribute__((aligned(32)))= {1.1, 2, 3, 2.5, 3.5, 4, 5.1, 8.4};
    __m256d v = _mm256_load_pd(values);
    double values2[8] __attribute__((aligned(32)))= {0.004, 0.1, 0.7, 0.6, 0.007, 0.08, 0.001, 0.08};
    __m256d v2 = _mm256_load_pd(values2);
    for(int i=0; i<truc;i++)
    {
        v = _mm256_fmadd_pd(v, v2, v);
    }
