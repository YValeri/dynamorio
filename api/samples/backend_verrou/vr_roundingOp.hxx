...
public:
  ...
  static inline RealType apply(const PackArgs& p ){
    ...
    //std::cout << "seed : " << vr_rand_getSeed(&vr_rand) << std::endl;
    ...
    if(signError==0.){
      ...
      //std::cout << "sign error : " << signError << "\tdonoChange : " << doNoChange << "\t";
      if(doNoChange){
        //std::cout << std::endl;
	return res;
      }else{
	if(signError>0){
    //std::cout << "nextAfter " << std::endl;
	  return nextAfter<RealType>(res);
	}else{
    //std::cout << "nextPrev " << std::endl;
