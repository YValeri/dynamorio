...
static void printbuffer(int num_src, int opcode)
{
    ...
    for(int i=0; i<10; i++)
        dr_printf("c[%d] : %02X, ", i, c[i]);
    *resultBuffer = 3.0 + *(dbuffer)+*(dbuffer+1);
    dr_printf("\nres : %lf\n",*(resultBuffer));
}
            ...
            if(opnd_is_reg(op))
            {
                ...
                if(reg_is_simd(reg)){ //SIMD scalar
                    ...
                    *(dbuffer+i) = (double)fpubuff;
                    translate_insert(INSTR_CREATE_fst(drcontext, opnd_create_rel_addr(&fpubuff, OPSZ_8)), 
                                    ilist, instr);
...
static dr_emit_flags_t event_basic_block(void *drcontext, void* tag, instrlist_t *bb, bool for_trace, bool translating)
{
    ...
    for(instr = instrlist_first(bb); instr != NULL; instr = next_instr)
    {
            ...
            || instr_get_opcode(instr)==OP_addsd)
        {
            ...
            dr_insert_clean_call(drcontext, bb, instr, (void*)printbuffer, false, 2, 
                INT2OPND(instr_num_srcs(instr)), INT2OPND(instr_get_opcode(instr)));
